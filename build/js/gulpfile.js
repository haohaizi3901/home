...
gulp.task('vendorjs', function () {
    var b = browserify().require('./bower_components/jquery/dist/jquery.js', { expose: 'jquery' }).bundle().pipe(fs.createWriteStream('vendor.js'));
});

gulp.task('babel', function () {
    gulp.src('*.js').pipe(babel({ presets: ["es2015"] })).pipe(gulp.dest('./build/js/'));
});
